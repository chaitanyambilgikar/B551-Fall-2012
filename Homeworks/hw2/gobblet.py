...
class GobbletState(GameState):
	...
	def copy_into(self, other):
		...
		GameState.copy_into(self, other)
		other.player = self.player
	...
	def is_valid_move(self, move):
		...
		if detail.source:
			...
			if detail.target == detail.source:
				return False
			#empty board
			if not self.board[s1][s2]:
                                return False
                        #no piece at source
                        if(self.board[s1][s2][-1].player != self.player \
					or self.board[s1][s2][-1].size != size):
                                return False
                        #lifting piece would result in a win
                        last = self.board[s1][s2].pop()
                        is_win = self.is_win(1-self.player)
                        self.board[s1][s2].append(last)
                        if is_win:
                                return False
                #no pieces remaining
                if not detail.source and self.pieces[self.player][size] <= 0:
			return False
		#destination has a piece that's too big
	...
	def successor_moves(self):
		...
		if self.isDraw:
			return []
		successors = GameState.successor_moves(self)
