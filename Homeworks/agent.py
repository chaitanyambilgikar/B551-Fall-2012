...
class Agent:
	...
	def update_goal_belief(self,mypose,bgoal,goalvisible):
		...
		"""
		print "My Location is ",(mypose[0],mypose[1])," and direction is ",mypose[2]
		...
		if goalvisible == 0:
			...
			number_correct_side = 0
			total_number_wrong_side = 0
			total_value_wrong_side = 0
			on_correct_side = []
			for (x,y),val in bgoal.iteritems():
				if val != 0:	
					if position_visible(mypose,(x,y),100):
						temp = position_visible(mypose,(x,y),100)
						if temp == goalvisible:
							number_correct_side+=1
							on_correct_side.append((x,y))
						else:
							total_number_wrong_side += 1
							total_value_wrong_side += val
							bgoal[(x,y)] = 0
			average_to_be_added = total_value_wrong_side/number_correct_side
			for (i,j) in on_correct_side:
				bgoal[(i,j)]+=average_to_be_added 
