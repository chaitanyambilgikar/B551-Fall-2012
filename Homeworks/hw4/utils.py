...
class FeatureExtractor():
	...
	def get_stopwords(self):
		...
		return self.create_bag_of_word_feature('EXIST_',stop_words)
...
class ProbDist():
	...
	def __init__(self, log=True, bin_boundaries=None):
		...
		for i in [self.val2binidx(v) for v in vlist]:
			cnts[i] = cnts.get(i, 0) + 1
		if self.log == True:
			self.data = dict([(i, math.log(( float(cnts[i]) + a - 1)/( len(vlist) + a + b -2 ))) for i in cnts.keys()])
		else:
			self.data = dict([(i, (float(cnts[i]) + a - 1)/(len(vlist) + a + b -2)) for i in cnts.keys()])
