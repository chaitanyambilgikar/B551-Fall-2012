...
def next_pose(map,pose,action):
	...
	if action == 'f':
		...
		if map == None or valid_cell(map,candidate):
			#print candidate[0],candidate[1],pose[2]
...
def next_pose_with_objects(map,pose,action,objects):
	...
	# nothing but comparing the next location to the location of all other objects. if match, then dont move
...
class Agent:
	...
	def update_goal_belief(self,mypose,bgoal,goalvisible):
		...
		#print "My Location is ",(mypose[0],mypose[1])," and direction is ",mypose[2]," goalvisible is ",goalvisible
		"""total_number_not_seen = 0
		total_value_not_seen = 0
		number_seen = 0
		for (x,y),val in bgoal.iteritems():
			if position_visible(mypose,(x,y),100) == 0:
				total_number_not_seen += 1
				total_value_not_seen += val
			else:
				number_seen+=1
				print (x,y),"is visible at ",position_visible(mypose,(x,y),100)
		print number_seen
		print goalvisible
		"""
		#print self.sensorModel.goalSensor.probability_raw()
		if goalvisible==0:
			for (x,y),val in bgoal.iteritems():
				if position_visible(mypose,(x,y),100):
					bgoal[(x,y)] = 0
		else:	
			for (x,y), val in bgoal.iteritems():
				prob = self.sensorModel.goalSensor.probability_raw(mypose,(x,y),goalvisible)
				if prob == 0:
					bgoal[(x,y)] = 0
		return bgoal.copy()
	...
	def update_object_belief(self,mypose,objname,bobject,obsvisible):
		...
		""" Here, probability_raw return a probability that there is an object at a particular cell,
		based on the visibily and the proximity sensor.
		Higher the probability, higher the chance of the object being there. 
		Iterate over all the cells, find the prob of an object being there(using probability_raw) and update
		bobject for that cell accordingly"""
		
		for each in bobject.keys():
			
			if bobject[each]!=0.0:
				bobject[each]*=self.sensorModel.objectSensors[objname].probability_raw(mypose,each,obsvisible)
			else:
				continue
		normalize(bobject)
		...
		"""
                ...
                #print "Object calling this is ",self.name," at ",mypose," other obj is ",objname
                #next_states =  self.agentBehaviorModels[objname].next_state_distribution(mypose)
                #for (a,b) in zip(next_states,bobject):
                #	print a,b
                update = bobject.copy()
                for k in bobject.keys():
                	next_states = self.agentBehaviorModels[objname].next_state_distribution(k)
                	for n in next_states.keys():
                		update[n] += next_states[n]*bobject[k]
                	#bobject[k] = update[n]
                return update
        ...
        def most_likely_trajectory(self,mypath,objname,objpercepts):
                ...
                for (mypose,objvisible) in zip(mypath,objpercepts)[1:]:
                        ...
                        transition = dict((s,[0.0,None]) for s in states)
                        #create a dictionary that would return max(transition*V[i-1,xi-1])
                        for k in Vlast.keys():
                            nxt_states = self.agentBehaviorModels[objname].next_state_distribution(k)
                            for st in nxt_states.keys():
                            		if (transition[st][0] <= (nxt_states[st]*Vlast[k])):
                            			transition[st][0] = (nxt_states[st]*Vlast[k])
                            			transition[st][1] = k
                        
                        for s in states:
                                #this line picks the previous state that maximizes the observation probability
                                best[s] = transition[s][1]
