...
class Agent:
	...
	def update_object_belief(self,mypose,objname,bobject,obsvisible):
        ...
        def predict_object_belief(self,mypose,objname,bobject):
			...
			"""for i in tempProbabilityDistribution:
				for j in bobject:
					s1 = 0.0
					s1 += bobject[j] * bobject[k]
					bobject[j] = (s1 * tempProbabilityDistribution[i])
					print bobject[j]
					#tempDist[j] += bobject[j] * tempDist[j]
			return tempDist"""
			for i in bobject:
        ...
        def most_likely_trajectory(self,mypath,objname,objpercepts):
                ...
                for (mypose,objvisible) in zip(mypath,objpercepts)[1:]:
                        ...
                        #print self.agentBehaviorModels[objname].next_state_distribution(mypath)
                        for s in states:
