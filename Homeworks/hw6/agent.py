...
class Agent:
	...
	def update_belief(self,b,observation,reward):
		...
		if reward!=0.0 and (bnext.mypose[0],bnext.mypose[1]) in bnext.b_goal:
                        del bnext.b_goal[(bnext.mypose[0],bnext.mypose[1])]
	...
	def update_goal_belief(self,mypose,bgoal,goalvisible):
		...
		#return bgoal.copy()
                #Here's my implementation, which uses probability values
                bgoal_next = dict()
                for goalpos,p in bgoal.iteritems():
                        bgoal_next[goalpos] = p*self.sensorModel.goalSensor.probability_raw(mypose,goalpos,goalvisible)
                        if bgoal_next[goalpos] == 0.0:
                                #speeds up subsequent inference steps...
                                del bgoal_next[goalpos]
                normalize(bgoal_next)
                return bgoal_next
	...
	def update_object_belief(self,mypose,objname,bobject,obsvisible):
		...
		#return bobject.copy()
                ...
                update_dist = dict()
                for (opose,p) in bobject.iteritems():
                        update_dist[opose] = p*self.sensorModel.objectSensors[objname].probability_raw(mypose,opose,obsvisible)
                for d in directions:
                        if (mypose[0],mypose[1],d) in update_dist:
                                del update_dist[(mypose[0],mypose[1],d)]
                normalize(update_dist)
                return update_dist
		...
		"""
                ...
                predict_dist = dict()
                for (opose,p) in bobject.iteritems():
                        nextdist = self.agentBehaviorModels[objname].next_state_distribution(opose)
                        for (onext,ponext) in nextdist.iteritems():
                                predict_dist[onext] = predict_dist.get(onext,0.0) + p*ponext
                return predict_dist
        ...
        def most_likely_trajectory(self,mypath,objname,objpercepts):
                ...
                for (mypose,objvisible) in zip(mypath,objpercepts)[1:]:
                        ...
                        for s in states:
                                ...
                                best[s] = max((v,k) for (k,v) in V.iteritems())[1]
                                ...
                                p[s] = obsprob*Vlast[best[s]]

                        # My implementation -- may be slightly more efficient than students' implementations 
                        """
                        bestprobs = dict((s,0.0) for s in states)
                        for sp in states:
                                #compute transition distribution from sp
                                ndist = self.agentBehaviorModels[objname].next_state_distribution(sp)
                                for (s,v) in ndist.iteritems():
                                        pv = Vlast[sp]*v
                                        if pv > bestprobs[s]:
                                                bestprobs[s] = pv
                                                best[s] = sp
		...
		bnext = self.update_belief(self.currentBelief,obs,rewards)
